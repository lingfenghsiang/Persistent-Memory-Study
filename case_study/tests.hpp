          ...
          int thread_num, bool helper_mod, int prereadnum,  std::string notion)
{
    ...
    finished_num.store(0);
    // uint64_t total_start_clock, total_end_clock;
    ...
    sync_buffer *shared_sync_buffer;
    ...
    std::cout << "[Throughput]:[" << keys_num / 1000.0 / duration.count() <<"]"<< " (Mops/s)" << std::endl;
    std::cout << "[Avg latency]:[" << avg_clock <<"]"<< " (CPU clocks)" << std::endl;
    std::cout << "[90% tail latency]:[" << latency.at((uint64_t)(keys_num * 0.9)) <<"]"<< " (CPU clocks)" << std::endl;
    std::cout << "[99% tail latency]:[" << latency.at((uint64_t)(keys_num * 0.99)) <<"]"<< " (CPU clocks)" << std::endl;
