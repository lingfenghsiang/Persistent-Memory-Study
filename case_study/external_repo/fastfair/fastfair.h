...
#endif
extern int adder;
extern int tmp_counter0, tmp_counter1;
...
#define PAGESIZE 512
#define ADDR_TRANS(x) ((x) / 12) * 16 + (x) % 12
...
#define IS_FORWARD(c) (c % 2 == 0)
    ...
    static inline void __attribute__((optimize("-O0"))) preread(void *addr)
    {
        char *read_addr = (char *)addr;
        char discard_content = *read_addr;
    }
...
// #endif
    ...
    //     void *address;
    //     // uint64_t time_stamp;
    //     // 0 for empty, 1 for being written, 2 for written & waiting to be fetched
    ...
    //         status.store(0);
    //     }
    //     hash_entry operator=(const  hash_entry &src){
    //         key = src.key;
    //         address = src.address;
    //     }
    // };

    // class unresizable_hashtable
    // {
    // #define SIZE 1 << 8
    // public:
    //     hash_entry the_table[SIZE];
    //     uint32_t fnv_hash(void *address, size_t length)
    //     {
    //         uint32_t prime = 4294966997L;
    //         uint32_t hash = 0xa673a21c;
    //         unsigned char *array = (unsigned char *)address;
    ...
    //         return hash;
    //     }
    //     bool get(uint64_t key, hash_entry &result)
    //     {
    //         int index = (SIZE - 1) & fnv_hash(&key, sizeof(uint64_t));
    //         int slot_status;
    //         for (;;)
    //         {
    //             slot_status = the_table[index].status.load();
    //             if (slot_status == 1)
    //             {
    //                 continue;
    //             }
    ...
    //             {
    //                 if (the_table[index].key == key)
    //                 {
    //                     result = the_table[index];
    //                     the_table[index].status.store(0);
    //                     return true;
    //                 }
    //                 else
    //                 {
    ...
    //         }
    //     }
    //     bool put(uint64_t key, void *address)
    //     {
    //         int index = (SIZE - 1) & fnv_hash(&key, sizeof(uint64_t));
    //         int slot_empty = 0;
    //         if (the_table[index].status.compare_exchange_strong(slot_empty, 1))
    //         {
    //             the_table[index].key = key;
    //             the_table[index].address = address;
    //             the_table[index].status.fetch_add(1);
    //             return true;
    //         }
    //         else
    //         {
    ...
    private:
        ...
        const int write_buffer_cl_size = 8;
        const int max_thread_num = 80;
        int max_cl = 0;
    ...
    public:
        ...
        void btree_spec_read(uint64_t, char *);
    ...
    public:
        ...
        bool remove_rebalancing(btree* bt, entry_key_t key, bool only_rebalance = false, bool with_lock = true) {
            ...
            if(total_num_entries > cardinality - 1) { // Redistribution
                ...
                for(int i = 0; records[i].ptr != NULL; ++i) { 
                    left_sibling->insert_key(bt,records[i].key, records[i].ptr, &left_num_entries);
                }
...
#endif
        inline void
        insert_key(btree *bt, uint64_t key, char *ptr, int *num_entries, bool flush = true,
                   bool update_last_index = true) __attribute__((optimize(0)))
        {
            ...
            else
            {
                ...
                cl_buffer_entry *cl_buffer_entry_array = (cl_buffer_entry *)(bt->write_buffer_region) + local_thread_id * bt->write_buffer_cl_size;
                cl_buffer_entry *dram_cl_buffer_entry_array = (cl_buffer_entry *)(bt->dram_write_buffer_region) + local_thread_id * bt->write_buffer_cl_size;
                cl_merged_entry *merged_entry_array;
                for (i = *num_entries - 1; i >= 0; i--)
                {
                    // to make sure that the code is 
                    if (key < records[i].key.ikey)
                    {
                        ...
                        if (flush)
                        {
                            ...
                            if (do_flush)
                            {
                                ...
                                // clflush((char *)records_ptr, sizeof(entry));
                                // to_flush_cnt = 0;
                            }
                            else
                                ++to_flush_cnt;
        ...
        inline void
        insert_key(btree *bt, key_item *key, char *ptr, int *num_entries, bool flush = true,
                   bool update_last_index = true) __attribute__((optimize(0)))
        {
            // update switch_counter
            if (!IS_FORWARD(hdr.switch_counter))
        ...
        page *store(btree *bt, char *left, uint64_t key, char *right,
                    bool flush, bool with_lock, page *invalid_sibling = NULL) __attribute__((optimize(0)))
        {
            ...
            { // FAIR
                ...
                { // leaf node
                    ...
                    {
                        sibling->insert_key(bt, records[i].key.ikey, records[i].ptr, &sibling_cnt, false);
                    }
                }
                else
                { // internal node
        ...
        page *store(btree *bt, char *left, key_item *key, char *right,
                    bool flush, bool with_lock, page *invalid_sibling = NULL) __attribute__((optimize(0)))
        {
            if (with_lock)
            {
            ...
            { // FAIR
                ...
                { // leaf node
                    for (int i = m; i < num_entries; ++i)
                    {
                        sibling->insert_key(bt, records[i].key.skey, records[i].ptr, &sibling_cnt, false);
                    }
                }
                else
                { // internal node
        ...
        void linear_search_range(uint64_t min, uint64_t max, unsigned long *buf, int num, int &off) __attribute__((optimize(0)))
        {
            int i;
            uint32_t previous_switch_counter;
            page *current = this;
        ...
        void linear_search_range(key_item *min, key_item *max, unsigned long *buf, int num, int &off) __attribute__((optimize(0)))
        {
            int i;
            uint32_t previous_switch_counter;
            page *current = this;
 ...
 */
...
#ifdef RUNNING_ON_PMM
        if ((write_buffer_region = vmem_aligned_alloc(vmp, 256, CACHE_LINE_SIZE * write_buffer_cl_size* max_thread_num)) == NULL)
        {
            perror("vmem_malloc");
            exit(1);
        }
        posix_memalign(&dram_write_buffer_region, 64, CACHE_LINE_SIZE * write_buffer_cl_size * max_thread_num);
        // dram_write_buffer_region = new char[CACHE_LINE_SIZE * write_buffer_cl_size * max_thread_num];
        // if (!dram_write_buffer_region)
        // {
        //     perror("malloc read addr");
        //     exit(1);
        // }
#endif
#ifndef RUNNING_ON_PMM
        posix_memalign(&write_buffer_region, 64, CACHE_LINE_SIZE * write_buffer_cl_size);
#endif
        height = 1;
    }
    ...
    key_item *btree::make_key_item(char *key, size_t key_len, bool flush)
    {
        void *aligned_alloc;
#ifdef RUNNING_ON_PMM
        if ((aligned_alloc = vmem_aligned_alloc(vmp, 256, sizeof(key_item) + key_len)) == NULL)
        {
            perror("vmem_malloc");
            exit(1);
        }
    ...
    { //need to be string
        page *p = (page *)root;
        // hash_entry result;
        // if (spec_ht.get(key, result))
        // {
        // p = (page *)result.address;
        // }
        // else
        // {
        while (p->hdr.leftmost_ptr != NULL)
    ...
    void btree::btree_spec_read(uint64_t key, char *right)
    { //need to be string
        page *p = (page *)root;
    ...
    { //need to be string
        ...
        if (!p->store(this, NULL, new_item, right, true, true))
        { // store
            btree_insert(key, right);
        }
    }
    ...
    void btree::btree_insert_internal(char *left, uint64_t key, char *right, uint32_t level)
    {
    ...
    void btree::btree_insert_internal(char *left, key_item *key, char *right, uint32_t level)
    {
        if (level > ((page *)root)->hdr.level)
            return;
